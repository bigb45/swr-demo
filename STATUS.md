...
_Last updated: April 2026 — homepage rewrite + catalog repository + Stitch surfaces + catalog filter redesign + video support_
...
- **Industrial Service & Repair Portal** (`/services/repair`): six service categories grid, four-step `RepairTimeline` (drop-off / diagnose / repair / pickup) with Ø 48 h turnaround callout, and a `RepairRequestForm` client component that builds a pre-filled `mailto:` payload — runs without backend wiring, swap the transport later.
...
- **My Fleet** (`/account/fleet`, `/account/fleet/[id]`): protected machine-fleet workspace for repeat customers. `src/lib/fleet.ts` exposes `Machine`, `MaintenanceRecord`, and a `FleetRepository` interface with a `demoRepository` (5 deterministic machines) toggled by `NEXT_PUBLIC_FLEET_DEMO=1` and an empty-by-default repository for production. `FleetMachineCard` lists brand/model/serial/warranty, `MaintenanceLogTable` renders the per-machine history with kind badges, and the detail page exposes quick repair / contact actions. New tile in the account dashboard and footer entry under the Company column.
...
- CMS plumbing: `magentoGet` now accepts an optional `storeCode`, `src/lib/cms.ts` fetches `cmsPage` / `cmsBlock` by identifier per locale, `CmsContent` sanitizes HTML via `isomorphic-dompurify`, and pages fall back gracefully to i18n copy when CMS content is missing. New marketing primitives in `src/components/marketing/` (`Hero`, `IndustryHero`, `Cta`, `FeatureGrid`, `ServiceCard`, `TrustStrip`, `BrandLogoStrip`, `ContactCard`, `FeaturedProductsRail`, `ServicePillarPage`, `CertificateCard`) power every new page.
...
| **Service pillars** `/services/{consulting,repair,delivery,customs}` | CMS-backed rich content + CTAs + consultation link. `/services/repair` is now an Industrial Service & Repair Portal (categories grid + timeline + repair-request form). `/services/customs` is a Swiss Delivery Center (zones, duty rates, "what we file for you" checklist, compliance-document deep links into `/catalog`) |
...
| **My Fleet** `/account/fleet` + `/account/fleet/[id]` | Protected machine fleet view with warranty status, maintenance log, and quick repair / contact actions. Empty-by-default repository today; `NEXT_PUBLIC_FLEET_DEMO=1` enables a 5-machine demo seed |
