...
interface AddToCartClusterProps {
  ...
  /** Optional ERP override from the PDP server fetch. */
  erp?: ErpSkuData | null;
}
...
export default function AddToCartCluster({
  ...
  const hasErpPrice =
    erp?.netAmount != null && Number.isFinite(erp.netAmount);
  const basePrice = hasErpPrice ? erp!.netAmount! : 0;
  const erpCurrency = erp?.currency?.trim() || "EUR";
