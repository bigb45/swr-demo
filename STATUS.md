...
- **Stakeholder feedback Sprint 2–4**: demo attribute hygiene (`product-display.ts`), partner SVG logos, homepage `CopilotHero` (replaces intent tiles), 4+ contextual Copilot chips + PDP context, `AccountHeaderButton` Tier 2, category-scoped facet scaffold (`category-filters.ts`), locale-scoped category names, `CONTENT-AUDIT.md` for C1/C2 gate.
