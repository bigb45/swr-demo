/**
 * GET /api/orders/[id]/shipments/[shipmentId]/pdf?locale=de
 *
 * Streams a Next.js-generated PDF delivery note for the currently signed-in
...
import { getShipmentForCustomer } from "@/lib/orders";
...
export async function GET(
  ...
  const result = await getShipmentForCustomer(id, shipmentId, token);
  if (!result) {
  ...
  const element = createElement(ShipmentPdf, {
    shipment: result.document,
    order: result.order,
    labels,
    locale,
  ...
  return new Response(webStream, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="SWR-delivery-${result.document.increment_id}.pdf"`,
